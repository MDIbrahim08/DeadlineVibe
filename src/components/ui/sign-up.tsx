...
import { ArrowRight, Mail, Gem, Lock, Eye, EyeOff, ArrowLeft, X, AlertCircle, PartyPopper, Loader, User, Calendar } from "lucide-react";
...
import { Component as SpotlightButton } from "./spotlight-button";
...
const GradientBackground = () => (
    <>
        ...
        <svg width="100%" height="100%" viewBox="0 0 800 600" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice" className="absolute top-0 left-0 w-full h-full bg-[#151518]">
            <defs>
                <linearGradient id="rev_grad1" x1="0%" y1="0%" x2="100%" y2="100%">
                    <stop offset="0%" style={{stopColor: '#c59a6c', stopOpacity: 0.8}} />
                    <stop offset="100%" style={{stopColor: '#f1d198', stopOpacity: 0.4}} />
                </linearGradient>
                <linearGradient id="rev_grad2" x1="0%" y1="0%" x2="100%" y2="100%">
                    <stop offset="0%" style={{stopColor: '#532152', stopOpacity: 0.9}} />
                    <stop offset="50%" style={{stopColor: '#84315d', stopOpacity: 0.7}} />
                    <stop offset="100%" style={{stopColor: '#2b1b36', stopOpacity: 0.6}} />
                </linearGradient>
                <radialGradient id="rev_grad3" cx="50%" cy="50%" r="50%">
                    <stop offset="0%" style={{stopColor: '#963640', stopOpacity: 0.8}} />
                    <stop offset="100%" style={{stopColor: '#301828', stopOpacity: 0.4}} />
                </radialGradient>
                <filter id="rev_blur1" x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur stdDeviation="60"/></filter>
                <filter id="rev_blur2" x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur stdDeviation="50"/></filter>
                <filter id="rev_blur3" x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur stdDeviation="70"/></filter>
            </defs>
            <g style={{ animation: 'float1 20s ease-in-out infinite' }}>
                <ellipse cx="150" cy="500" rx="400" ry="250" fill="url(#rev_grad1)" filter="url(#rev_blur1)" transform="rotate(-20 150 500)"/>
                <rect x="550" y="50" width="450" height="350" rx="150" fill="url(#rev_grad2)" filter="url(#rev_blur2)" transform="rotate(15 650 225)"/>
            </g>
            <g style={{ animation: 'float2 25s ease-in-out infinite' }}>
                <circle cx="700" cy="500" r="300" fill="url(#rev_grad3)" filter="url(#rev_blur3)" opacity="0.6"/>
            </g>
        </svg>
    </>
);
...
export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "EaseMize" }: AuthComponentProps) => {
  ...
  const isOtpValid = otp.length >= 6 && otp.length <= 8;
  ...
  const handleGoBack = () => {
    if (authStep === 'confirmPassword') {
        ...
        if (isLoginMode) setAuthStep('email');
        else setAuthStep('dob');
    }
    else if (authStep === 'dob') setAuthStep('name');
    else if (authStep === 'name') setAuthStep('email');
  };
...
useEffect(() => {
    ...
    else if (authStep === 'dob') setTimeout(() => dobInputRef.current?.focus(), 500);
    else if (authStep === 'otp') setTimeout(() => otpInputRef.current?.focus(), 500);
...
useEffect(() => {
    ...
    <AnimatePresence>
        {modalStatus !== 'closed' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-black/80 border-4 border-slate-800 rounded-2xl p-8 w-full max-w-sm flex flex-col items-center gap-4 mx-2">
  ...
  return (
    <div className="bg-black min-h-screen w-screen flex flex-col">
        <style>{`
            input[type="password"]::-ms-reveal, input[type="password"]::-ms-clear { display: none !important; } input[type="password"]::-webkit-credentials-auto-fill-button, input[type="password"]::-webkit-strong-password-auto-fill-button { display: none !important; } input:-webkit-autofill, input:-webkit-autofill:hover, input:-webkit-autofill:focus, input:-webkit-autofill:active { -webkit-box-shadow: 0 0 0 30px transparent inset !important; -webkit-text-fill-color: #fff !important; background-color: transparent !important; background-clip: content-box !important; transition: background-color 5000s ease-in-out 0s !important; color: #fff !important; caret-color: #fff !important; } input:autofill { background-color: transparent !important; background-clip: content-box !important; -webkit-text-fill-color: #fff !important; color: #fff !important; } input:-internal-autofill-selected { background-color: transparent !important; background-image: none !important; color: #fff !important; -webkit-text-fill-color: #fff !important; } input:-webkit-autofill::first-line { color: #fff !important; -webkit-text-fill-color: #fff !important; }
            ...
            .glass-button { -webkit-tap-highlight-color: transparent; backdrop-filter: blur(clamp(1px, 0.125em, 4px)); transition: all var(--anim-time) var(--anim-ease); background: linear-gradient(-75deg, oklch(from #000 l c h / 5%), oklch(from #000 l c h / 20%), oklch(from #000 l c h / 5%)); box-shadow: inset 0 0.125em 0.125em oklch(from #fff l c h / 5%), inset 0 -0.125em 0.125em oklch(from #000 l c h / 50%), 0 0.25em 0.125em -0.125em oklch(from #fff l c h / 20%), 0 0 0.1em 0.25em inset oklch(from #000 l c h / 20%), 0 0 0 0 oklch(from #000 l c h); } .glass-button:hover { transform: scale(0.975); backdrop-filter: blur(0.01em); box-shadow: inset 0 0.125em 0.125em oklch(from #fff l c h / 5%), inset 0 -0.125em 0.125em oklch(from #000 l c h / 50%), 0 0.15em 0.05em -0.1em oklch(from #fff l c h / 25%), 0 0 0.05em 0.1em inset oklch(from #000 l c h / 50%), 0 0 0 0 oklch(from #000 l c h); } .glass-button-text { color: oklch(from #fff l c h / 90%); text-shadow: 0em 0.25em 0.05em oklch(from #fff l c h / 10%); transition: all var(--anim-time) var(--anim-ease); } .glass-button:hover .glass-button-text { text-shadow: 0.025em 0.025em 0.025em oklch(from #fff l c h / 12%); } .glass-button-text::after { content: ""; display: block; position: absolute; width: calc(100% - var(--border-width)); height: calc(100% - var(--border-width)); top: calc(0% + var(--border-width) / 2); left: calc(0% + var(--border-width) / 2); box-sizing: border-box; border-radius: 9999px; overflow: clip; background: linear-gradient(var(--angle-2), transparent 0%, oklch(from #000 l c h / 50%) 40% 50%, transparent 55%); z-index: 3; mix-blend-mode: screen; pointer-events: none; background-size: 200% 200%; background-position: 0% 50%; transition: background-position calc(var(--anim-time) * 1.25) var(--anim-ease), --angle-2 calc(var(--anim-time) * 1.25) var(--anim-ease); } .glass-button:hover .glass-button-text::after { background-position: 25% 50%; } .glass-button:active .glass-button-text::after { background-position: 50% 15%; --angle-2: -15deg; } .glass-button::after { content: ""; position: absolute; z-index: 1; inset: 0; border-radius: 9999px; width: calc(100% + var(--border-width)); height: calc(100% + var(--border-width)); top: calc(0% - var(--border-width) / 2); left: calc(0% - var(--border-width) / 2); padding: var(--border-width); box-sizing: border-box; background: conic-gradient(from var(--angle-1) at 50% 50%, oklch(from #fff l c h / 50%) 0%, transparent 5% 40%, oklch(from #fff l c h / 50%) 50%, transparent 60% 95%, oklch(from #fff l c h / 50%) 100%), linear-gradient(180deg, oklch(from #000 l c h / 50%), oklch(from #000 l c h / 50%)); mask: linear-gradient(#000 0 0) content-box, linear-gradient(#000 0 0); mask-composite: exclude; transition: all var(--anim-time) var(--anim-ease), --angle-1 500ms ease; box-shadow: inset 0 0 0 calc(var(--border-width) / 2) oklch(from #000 l c h / 50%); pointer-events: none; } .glass-button:hover::after { --angle-1: -125deg; } .glass-button:active::after { --angle-1: -75deg; } .glass-button-wrap:has(.glass-button:hover) .glass-button-shadow { filter: blur(clamp(2px, 0.0625em, 6px)); } .glass-button-wrap:has(.glass-button:hover) .glass-button-shadow::after { top: calc(var(--shadow-cutoff-fix) - 0.875em); opacity: 1; } .glass-button-wrap:has(.glass-button:active) .glass-button-shadow { filter: blur(clamp(2px, 0.125em, 12px)); } .glass-button-wrap:has(.glass-button:active) .glass-button-shadow::after { top: calc(var(--shadow-cutoff-fix) - 0.5em); opacity: 0.75; } .glass-button-wrap:has(.glass-button:active) .glass-button-text { text-shadow: 0.025em 0.25em 0.05em oklch(from #fff l c h / 12%); } .glass-button-wrap:has(.glass-button:active) .glass-button { box-shadow: inset 0 0.125em 0.125em oklch(from #fff l c h / 5%), inset 0 -0.125em 0.125em oklch(from #000 l c h / 50%), 0 0.125em 0.125em -0.125em oklch(from #fff l c h / 20%), 0 0 0.1em 0.25em inset oklch(from #000 l c h / 20%), 0 0.225em 0.05em 0 oklch(from #fff l c h / 5%), 0 0.25em 0 0 oklch(from #000 l c h / 75%), inset 0 0.25em 0.05em 0 oklch(from #fff l c h / 15%); } @media (hover: none) and (pointer: coarse) { .glass-button::after, .glass-button:hover::after, .glass-button:active::after { --angle-1: -75deg; } .glass-button .glass-button-text::after, .glass-button:active .glass-button-text::after { --angle-2: -45deg; } }
            .glass-input-wrap { position: relative; z-index: 2; transform-style: preserve-3d; border-radius: 9999px; } .glass-input { display: flex; position: relative; width: 100%; align-items: center; gap: 0.5rem; border-radius: 9999px; padding: 0.25rem; -webkit-tap-highlight-color: transparent; backdrop-filter: blur(clamp(1px, 0.125em, 4px)); transition: all 400ms cubic-bezier(0.25, 1, 0.5, 1); background: linear-gradient(-75deg, oklch(from #000 l c h / 5%), oklch(from #000 l c h / 20%), oklch(from #000 l c h / 5%)); box-shadow: inset 0 0.125em 0.125em oklch(from #fff l c h / 5%), inset 0 -0.125em 0.125em oklch(from #000 l c h / 50%), 0 0.25em 0.125em -0.125em oklch(from #fff l c h / 20%), 0 0 0.1em 0.25em inset oklch(from #000 l c h / 20%), 0 0 0 0 oklch(from #000 l c h); } .glass-input-wrap:focus-within .glass-input { backdrop-filter: blur(0.01em); box-shadow: inset 0 0.125em 0.125em oklch(from #fff l c h / 5%), inset 0 -0.125em 0.125em oklch(from #000 l c h / 50%), 0 0.15em 0.05em -0.1em oklch(from #fff l c h / 25%), 0 0 0.05em 0.1em inset oklch(from #000 l c h / 50%), 0 0 0 0 oklch(from #000 l c h); } .glass-input::after { content: ""; position: absolute; z-index: 1; inset: 0; border-radius: 9999px; width: calc(100% + clamp(1px, 0.0625em, 4px)); height: calc(100% + clamp(1px, 0.0625em, 4px)); top: calc(0% - clamp(1px, 0.0625em, 4px) / 2); left: calc(0% - clamp(1px, 0.0625em, 4px) / 2); padding: clamp(1px, 0.0625em, 4px); box-sizing: border-box; background: conic-gradient(from var(--angle-1) at 50% 50%, oklch(from #fff l c h / 50%) 0%, transparent 5% 40%, oklch(from #fff l c h / 50%) 50%, transparent 60% 95%, oklch(from #fff l c h / 50%) 100%), linear-gradient(180deg, oklch(from #000 l c h / 50%), oklch(from #000 l c h / 50%)); mask: linear-gradient(#000 0 0) content-box, linear-gradient(#000 0 0); mask-composite: exclude; transition: all 400ms cubic-bezier(0.25, 1, 0.5, 1), --angle-1 500ms ease; box-shadow: inset 0 0 0 calc(clamp(1px, 0.0625em, 4px) / 2) oklch(from #000 l c h / 50%); pointer-events: none; } .glass-input-wrap:focus-within .glass-input::after { --angle-1: -125deg; } .glass-input-text-area { position: absolute; inset: 0; border-radius: 9999px; pointer-events: none; } .glass-input-text-area::after { content: ""; display: block; position: absolute; width: calc(100% - clamp(1px, 0.0625em, 4px)); height: calc(100% - clamp(1px, 0.0625em, 4px)); top: calc(0% + clamp(1px, 0.0625em, 4px) / 2); left: calc(0% + clamp(1px, 0.0625em, 4px) / 2); box-sizing: border-box; border-radius: 9999px; overflow: clip; background: linear-gradient(var(--angle-2), transparent 0%, oklch(from #000 l c h / 50%) 40% 50%, transparent 55%); z-index: 3; mix-blend-mode: screen; pointer-events: none; background-size: 200% 200%; background-position: 0% 50%; transition: background-position calc(400ms * 1.25) cubic-bezier(0.25, 1, 0.5, 1), --angle-2 calc(400ms * 1.25) cubic-bezier(0.25, 1, 0.5, 1); } .glass-input-wrap:focus-within .glass-input-text-area::after { background-position: 25% 50%; }
        `}</style>
            ...
            <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center gap-8 w-[280px] mx-auto p-4">
                <AnimatePresence mode="wait">
                    {authStep === "email" && <motion.div key="email-content" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center gap-4">
                        <BlurFade delay={0.25 * 1} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl md:text-6xl tracking-tight text-white whitespace-nowrap">{isLoginMode ? "Welcome Back" : "Create Account"}</p></div></BlurFade>
                    </motion.div>}
                    {authStep === "name" && <motion.div key="name-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                        <BlurFade delay={0} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-white whitespace-nowrap">Your Name</p></div></BlurFade>
                        <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-slate-400">What should we call you?</p></BlurFade>
                    </motion.div>}
                    {authStep === "dob" && <motion.div key="dob-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                        <BlurFade delay={0} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-white whitespace-nowrap">Birth Date</p></div></BlurFade>
                        <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-slate-400">When were you born?</p></BlurFade>
                    </motion.div>}
                    {authStep === "otp" && <motion.div key="otp-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                        <BlurFade delay={0} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-white whitespace-nowrap">Verify Email</p></div></BlurFade>
                        <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-slate-400">We sent a verification code to {email}</p></BlurFade>
                    </motion.div>}
                    {authStep === "password" && <motion.div key="password-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                        <BlurFade delay={0} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-white whitespace-nowrap">Enter Password</p></div></BlurFade>
                        <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-slate-400">Log in or finish creating your account.</p></BlurFade>
                    </motion.div>}
                     {authStep === "confirmPassword" && <motion.div key="confirm-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                         ...
                         <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-slate-400">Confirm your password to continue</p></BlurFade>
                    </motion.div>}
                </AnimatePresence>
                ...
                <form onSubmit={handleFinalSubmit} className="w-[300px] space-y-6">
                        ...
                        {authStep !== 'confirmPassword' && <motion.div key="email-password-fields" exit={{ opacity: 0, filter: 'blur(4px)' }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full space-y-6">
                            <BlurFade delay={authStep === 'email' ? 0.25 * 5 : 0} inView={true} className="w-full">
                                <div className="relative w-full">
                                    <AnimatePresence>
                                        {authStep === "password" && <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3, delay: 0.4 }} className="absolute -top-6 left-4 z-10"><label className="text-xs text-slate-400 font-semibold">Email</label></motion.div>}
                                    </AnimatePresence>
                                ...
                                <div className="mt-6 flex flex-col items-center gap-3 w-full">
                                    ...
                                    <SpotlightButton type="button" onClick={() => setIsLoginMode(!isLoginMode)} className="w-full">
                            ...
                            <AnimatePresence>
                                {authStep === "name" && <BlurFade key="name-field" className="w-full">
                                    <div className="relative w-full">
                                        <AnimatePresence>
                                            {name.length > 0 && <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3 }} className="absolute -top-6 left-4 z-10"><label className="text-xs text-slate-400 font-semibold">Name</label></motion.div>}
                                        </AnimatePresence>
                                        <div className="glass-input-wrap w-full"><div className="glass-input">
                                            <span className="glass-input-text-area"></span>
                                            ...
                                            <input ref={nameInputRef} type="text" placeholder="Your Name" value={name} onChange={(e) => setName(e.target.value)} onKeyDown={handleKeyDown} className="relative z-10 h-full w-0 flex-grow bg-transparent text-white placeholder:text-white/60 focus:outline-none" />
                                            <div className={cn( "relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isNameValid ? "w-10 pr-1" : "w-0" )}><GlassButton type="button" onClick={handleProgressStep} size="icon" aria-label="Submit name" contentClassName="text-white/80 hover:text-white"><ArrowRight className="w-5 h-5" /></GlassButton></div>
                                        </div></div>
                                    </div>
                                    <BlurFade inView delay={0.2}><div className="mt-6 flex justify-center"><SpotlightButton type="button" onClick={handleGoBack} className="flex items-center gap-2"><ArrowLeft className="w-4 h-4" /> Go back</SpotlightButton></div></BlurFade>
                                </BlurFade>}
                            </AnimatePresence>
                            <AnimatePresence>
                                {authStep === "dob" && <BlurFade key="dob-field" className="w-full">
                                    <div className="relative w-full">
                                        <AnimatePresence>
                                            {dob.length > 0 && <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3 }} className="absolute -top-6 left-4 z-10"><label className="text-xs text-slate-400 font-semibold">Date of Birth</label></motion.div>}
                                        </AnimatePresence>
                                        <div className="glass-input-wrap w-full"><div className="glass-input">
                                            <span className="glass-input-text-area"></span>
                                            <div className="relative z-10 flex-shrink-0 flex items-center justify-center w-10 pl-2">
                                                <Calendar className="h-5 w-5 text-white/80 flex-shrink-0" />
                                            </div>
                                            <input ref={dobInputRef} type="date" value={dob} onChange={(e) => setDob(e.target.value)} onKeyDown={handleKeyDown} className="relative z-10 h-full w-0 flex-grow bg-transparent text-white placeholder:text-white/60 focus:outline-none [color-scheme:dark]" />
                                            <div className={cn( "relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isDobValid ? "w-10 pr-1" : "w-0" )}><GlassButton type="button" onClick={handleProgressStep} size="icon" aria-label="Submit date of birth" contentClassName="text-white/80 hover:text-white"><ArrowRight className="w-5 h-5" /></GlassButton></div>
                                        </div></div>
                                    </div>
                                    <BlurFade inView delay={0.2}><div className="mt-6 flex justify-center"><SpotlightButton type="button" onClick={handleGoBack} className="flex items-center gap-2"><ArrowLeft className="w-4 h-4" /> Go back</SpotlightButton></div></BlurFade>
                                </BlurFade>}
                            ...
                            <AnimatePresence>
                                {authStep === "otp" && <BlurFade key="otp-field" className="w-full">
                                    <div className="relative w-full">
                                        <AnimatePresence>
                                            {otp.length > 0 && <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3 }} className="absolute -top-6 left-4 z-10"><label className="text-xs text-slate-400 font-semibold">Verification Code</label></motion.div>}
                                        </AnimatePresence>
                                        <div className="glass-input-wrap w-full"><div className="glass-input">
                                            <span className="glass-input-text-area"></span>
                                            <div className="relative z-10 flex-shrink-0 flex items-center justify-center w-10 pl-2">
                                                <Lock className="h-5 w-5 text-white/80 flex-shrink-0" />
                                            </div>
                                            <input ref={otpInputRef} type="text" placeholder="Verification code" value={otp} onChange={(e) => setOtp(e.target.value)} onKeyDown={handleKeyDown} className="relative z-10 h-full w-0 flex-grow bg-transparent text-white placeholder:text-white/60 focus:outline-none tracking-widest" maxLength={8} />
                                            <div className={cn( "relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isOtpValid ? "w-10 pr-1" : "w-0" )}><GlassButton type="button" onClick={handleVerifyOtp} size="icon" aria-label="Submit OTP" contentClassName="text-white/80 hover:text-white"><ArrowRight className="w-5 h-5" /></GlassButton></div>
                                        </div></div>
                                    </div>
                                    <BlurFade inView delay={0.2}><div className="mt-6 flex justify-center"><SpotlightButton type="button" onClick={() => setAuthStep('email')} className="flex items-center gap-2"><ArrowLeft className="w-4 h-4" /> Start Over</SpotlightButton></div></BlurFade>
                                </BlurFade>}
                            </AnimatePresence>
                            <AnimatePresence>
                                {authStep === "password" && <BlurFade key="password-field" className="w-full">
                                    <div className="relative w-full">
                                        <AnimatePresence>
                                            {password.length > 0 && <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3 }} className="absolute -top-6 left-4 z-10"><label className="text-xs text-slate-400 font-semibold">Password</label></motion.div>}
                                        </AnimatePresence>
                                        <div className="glass-input-wrap w-full"><div className="glass-input">
                                            <span className="glass-input-text-area"></span>
                                            <div className="relative z-10 flex-shrink-0 flex items-center justify-center w-10 pl-2">
                                            ...
                                            <div className={cn( "relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isPasswordValid ? "w-10 pr-1" : "w-0" )}><GlassButton type={isLoginMode ? "submit" : "button"} onClick={!isLoginMode ? handleProgressStep : undefined} size="icon" aria-label="Submit password" contentClassName="text-white/80 hover:text-white"><ArrowRight className="w-5 h-5" /></GlassButton></div>
                                        </div></div>
                                    </div>
                                    <BlurFade inView delay={0.2}><div className="mt-6 flex justify-center"><SpotlightButton type="button" onClick={handleGoBack} className="flex items-center gap-2"><ArrowLeft className="w-4 h-4" /> Go back</SpotlightButton></div></BlurFade>
                                </BlurFade>}
                    ...
                    <AnimatePresence>
                        {authStep === 'confirmPassword' && <BlurFade key="confirm-password-field" className="w-full">
                            <div className="relative w-full">
                                <AnimatePresence>
                                    {confirmPassword.length > 0 && <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3 }} className="absolute -top-6 left-4 z-10"><label className="text-xs text-slate-400 font-semibold">Confirm Password</label></motion.div>}
                                </AnimatePresence>
                                <div className="glass-input-wrap w-[300px]"><div className="glass-input">
                                    ...
                                    <div className={cn( "relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isConfirmPasswordValid ? "w-10 pr-1" : "w-0" )}><GlassButton type="submit" size="icon" aria-label="Finish sign-up" contentClassName="text-white/80 hover:text-white"><ArrowRight className="w-5 h-5" /></GlassButton></div>
                                </div></div>
                            </div>
                            <BlurFade inView delay={0.2}><div className="mt-6 flex justify-center"><SpotlightButton type="button" onClick={handleGoBack} className="flex items-center gap-2"><ArrowLeft className="w-4 h-4" /> Go back</SpotlightButton></div></BlurFade>
                        </BlurFade>}
